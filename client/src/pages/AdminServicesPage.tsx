  ...
  return (
    <section className="container mx-auto p-4">
      ...
      <div className="overflow-x-auto rounded-lg shadow">
        <div className="min-w-full grid grid-cols-7 gap-x-4 bg-gray-100 p-2 text-sm font-semibold text-gray-700">
          <div>ID</div>
          <div>Name</div>
          <div>Category</div>
          <div>Price</div>
        ...
        {services.map(s => (
          <div key={s.id} className="min-w-full grid grid-cols-7 gap-x-4 border-b p-2 items-center text-sm">
            ...
            <div>{s.averageRating.toFixed(1)} / 5</div>
        ...
        {!services.length && (
          <div className="p-4 text-center text-gray-600">No unapproved services found.</div>
        )}
      </div>
    </section>
  );
